...
%ABSTRACTODEFLOW2D Abstract methods for 2D plots of velocity field.
  ...
  methods
    ...
    %
    % ftle(obj, tau, PARAMETER, VALUE, PARAMETER, VALUE,...)
    %
    % tau - duration over which FTLE is computed
    % Optional parameters:
    % R - resolution of the grid (if autogenerated)
    % xi, yi - manually generated computational grid
    % t0 - initial time at which ftle is calculated
    ...
    % Initialize
      ...
      [p,t] = obj.flow(p0, tau, t0 );
      
      % compute terminal points
      pend = squeeze( p(:,end,:) );
      Xt = reshape(pend(1,:),size(X));
      Yt = reshape(pend(2,:),size(X));
      
      % compute gradient of the flow map
      [dXtdX, dXtdY] = gradient(Xt, xi, yi);
      [dYtdX, dYtdY] = gradient(Yt, xi, yi);
      
      % form the jacobian
      J = zeros([2,2,numel(yi), numel(xi)]);
      J(1,1,:,:) = dXtdX;
      J(1,2,:,:) = dXtdY;
      J(2,1,:,:) = dYtdX;
      J(2,2,:,:) = dYtdY;
      ...
      for idx = 1:numel(xi)
          for idy = 1:numel(yi)
            FTLE(idy, idx) = log(norm(J(:,:,idy,idx),2))/abs(tau);              
          end
      end
